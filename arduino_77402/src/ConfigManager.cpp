...
{
		//Clear EEPROM before startup
		for (int i = 0 ; i < CONFIG_OFFSET + this->configSize ; i++) {
			EEPROM.write(i, 0);
		}
...
void ConfigManager::handleAPPost() {
    ...
    if (isJson) {
		...
		homeName = obj.get<String>("homeName");
		homePassword = obj.get<String>("homePassword");
    } else {
        ssid = server->arg("ssid");
        password = server->arg("password");
		homeName = server->arg("homeName");
		homePassword = server->arg("homePassword");
    }
	...
	EEPROM.put(WIFI_OFFSET + 96, homeName);
	EEPROM.put(WIFI_OFFSET + 196, homePassword);
    EEPROM.commit();
