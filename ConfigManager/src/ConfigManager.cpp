...
String ConfigManager::getHomeName(){
	return this->homeName;
}

String ConfigManager::getHomePassword(){
...
void ConfigManager::restartChip(){
    ESP.restart();
}
...
void ConfigManager::handleAPPost() {
    ...
    String password;
    String homeNameTemp;
    String homePasswordTemp;
    char ssidChar[32];
    char passwordChar[64];
    char homeNameChar[100];
    char homePasswordChar[100];
    ...
    if (isJson) {
		...
		homeNameTemp = obj.get<String>("homeName");
		homePasswordTemp = obj.get<String>("homePassword");
    } else {
        ssid = server->arg("ssid");
        password = server->arg("password");
		homeNameTemp = server->arg("homeName");
		homePasswordTemp = server->arg("homePassword");
    }
    ...
    strncpy(homeNameChar, homeNameTemp.c_str(), sizeof(homeName));
    strncpy(homePasswordChar, homePasswordTemp.c_str(), sizeof(homePassword));


	//Write Wi-Fi credentials and home data to EEPROM
    EEPROM.put(0, magicBytes);
    EEPROM.put(WIFI_OFFSET, ssidChar);
    EEPROM.put(WIFI_OFFSET + 32, passwordChar);
	EEPROM.put(WIFI_OFFSET + 96, homeNameChar);
	EEPROM.put(WIFI_OFFSET + 196, homePasswordChar);
...
void ConfigManager::setup() {
	...
	//Read in saved home data and Wi-Fi credentials
    Serial.println(F("Reading saved configuration"));

    EEPROM.get(0, magic);
    EEPROM.get(WIFI_OFFSET, ssid);
    EEPROM.get(WIFI_OFFSET + 32, password);
    EEPROM.get(WIFI_OFFSET + 96, homeName);
	EEPROM.get(WIFI_OFFSET + 196, homePassword);
    readConfig();
